...
- 非静态内容的this都是指向实例，而静态内容的this指向类。  
  类中非静态的内容都是给实例用的，而静态的内容都是给类用的。  
- 类构造函数也可以`return`一个对象，然后实例就会是这个对象
...
## 类的继承
子类会继承父类的方法（继承来的方法的表现和子类方法的表现一致）
不写构造函数时会默认执行父类的构造函数
- `super`  
  - 功能  
    执行super意味着给父类构造函数传参后执行，并添加父类属性、方法，最终创建this（所以super前不能用this，不然会阻塞报错）  
    子类中可以用super调用父类静态方法  
    在构造函数里`super.父类静态方法`只会返回undefined，不过在子类中可以用`super.父类静态方法`（在`super()`前`super.父类静态方法`的话会阻塞报错）  
  - 使用条件  
