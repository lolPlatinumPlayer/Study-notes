...
- `component`的值是组件
  - 项目中一般用异步加载  
    写法如下  
    `component: () => import('@/路径.vue')`  
    <span style='opacity:.5'>（这个功能来自webpack，可以进webpack笔记查看更多内容）</span>
...
- param也是可以不存在url里的  
  但是不存的param在刷新后就没有了<span style='opacity:.5'>（有存的刷新后还会在）</span>



### [动态路径参数](https://v3.router.vuejs.org/zh/guide/essentials/dynamic-matching.html#%E5%8A%A8%E6%80%81%E8%B7%AF%E7%94%B1%E5%8C%B9%E9%85%8D)
...
`children`属性代表嵌套路由，子项语法和routes完全一致

- 子路由对应的url  
  - 如果希望子路由对应的url就是path值的话  
    path值要以`/`开头
  - 如果希望子路由对应的url是父路由url加上path值的话  
    path值不能以`/`开头
- 渲染  
  父路由的`template`都会渲染，父路由`template`的`router-view`中会渲染出子路由的`template`

？？？以上两条适用于所有与path同级的路由属性（已验证：重定向redirect、别名alias【目前只学了这两个】）
...
- 添加多条第一级路由  
  [`router.addRoutes`](https://v3.router.vuejs.org/zh/api/#router-addroutes)
- 添加一条路由  
  [`router.addRoute`](https://v3.router.vuejs.org/zh/api/#router-addroute)  
  - 添加一条第一级路由  
    `router.addRoute(路由配置对象)`
  - 添加一条子路由  
    `router.addRoute(父路由的name,路由配置对象)`
  - 如果新路由的`name`和某个旧路由一致，那新路由会覆盖旧路由
