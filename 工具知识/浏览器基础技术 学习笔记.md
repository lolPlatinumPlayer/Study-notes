...
- img
  ...
  - 更改img的src后，图片有时要等一段时间才会更新  
    解决办法：改src就把整个img删掉，重新建个img放新的src  
    原因：新src在加载期间显示的还是旧src的图片
...
- 可展开内容  
  用[`details`标签](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/details)和`summary`标签的组合实现
  - 开启或关闭状态  
    都是用`details`标签的`oepn`属性的存在来表示  
    包括用户通过点击来开关也会修改`oepn`属性的存在
  - 标题  
    `summary`标签  
    不管开启还是关闭都会存在

- title标签的文本  
  下面2种方式目前感觉是一致的，包括设置和读取
  
  - `document.title`<span style='opacity:.5'>（没了解这是不是最佳方式）</span>
  - `document.getElementsByTagName('title')[0].innerHTML`
  
  在网页里修改title标签的文本就能改变浏览器标签页的标题  
  但是在app的webview里无法改变显示的标题（试过安卓的莆田惠民宝app）
...
- 单个元素
  [`document.querySelector(字符串格式的css选择器)`](https://developer.mozilla.org/zh-CN/docs/Web/API/Document/querySelector)   
  只会返回匹配到的第一个元素
- 多个元素
...
- 获得dom内部的html代码<span style='opacity:.5'>（不包括dom的标签本身）</span>  
  `innerHTML`
- 获得dom全部的html代码  
  `outerHTML`<span style='opacity:.5'>（已测试chrome101）</span>
- 将html代码转为dom后插入文档
  - 替换某个dom的后代  
    给`innerHTML`赋值
  - 给某个dom增加后代  
...
- [`document.write`](https://developer.mozilla.org/zh-CN/docs/Web/API/Document/write)  
  第一次写会替换掉body里的内容  
  后续使用会继续往body的末尾追加内容  
  - 直到调用[`document.close()`](https://developer.mozilla.org/zh-CN/docs/Web/API/Document/close)  
    调用`document.close()`后再调用`document.write`会和第一次写一样替换掉body里的内容
...
<span style='opacity:.5'>（这里只拿chrome99的高度来举例）</span>

已测试的高度：`dom.clientHeight`、`dom.offsetHeight`、`dom.scrollHeight`、`dom.getBoundingClientRect().height`

- 只有`dom.scrollHeight`的表现和其他高度不一样  
  - 其他高度都是真正的dom高度
  - `dom.scrollHeight`是dom内部的高度  
    和dom真正的高度没有关系<span style='opacity:.5'>（在有`overflow:hidden;`的情况下缩减height并不会影响`dom.scrollHeight`）</span>  
    <span style='opacity:.5'>（如果有做`overflow:hidden;`这种设置，那后代的margin也会占据`dom.scrollHeight`）</span>
...
- 待研究的缓动页面  
  - [java官网](https://dev.java/learn/getting-started-with-java/#setting-up-a-jdk-for-windowsx64)
...
- 尺寸  
  ...
  - 拉伸  
    `object-fit: fill;`
  - contain和scale-down区别  
    在图片过小时contain会放大而scale-down不会
...
可以像操作dom一样操作文档片段，但是文档片段并不是dom，不会导致一些性能问题

- 对文档片段调用dom方法  
  对文档片段调用dom方法就相当于对文档片段的所有子元素调用改方法
- 把文档片段传入`appendChild`方法会清空文档片段的子元素  
  <span style='opacity:.5'>（因为这相当与对所有子元素使用`appendChild`方法，而`appendChild`的作用就是把dom从旧的父级移动到新的父级上）</span>
...
- 多样化地控制元素的可见区域  
  ...
  - 对border、内阴影这种东西没有影响
- 内阴影  
  `box-shadow: inset 0px 0px 14px 1px green;`
- border样式  
...
- relative元素可以相对于自身进行偏移  
  用left、top、right、bottom
...
- 似乎会受到祖先的transform影响  
  <span style='opacity:.5'>（element全屏查看图片、ViewerJS全屏看图片就会受到影响）</span>
...
[火狐应该无法隐藏滚动条，最多是设为透明](https://blog.csdn.net/weixin_30611509/article/details/99315601)
...
- [渐变色](https://developer.mozilla.org/zh-CN/docs/Web/CSS/CSS_Images/Using_CSS_gradients)  
  源自`background-image`（但是`background`上也可以写，因为`background`上可以写`background-image`）  
  可以同时上多条渐变（让它们叠加在一起）
- background-image应该永远会盖在background-color上
- `background-position`  
  - 写2个参数时  
  ...
  - 百分比  
    0%和100%都不会让背景图超出边界
- 多重背景  
  兼容性挺好的，IE9开始支持，其他浏览器基本一开始就支持
- background-size和background-position一起写  
  background-size只能写在background-position后面，中间用`/`分割，比如`left top / 15% 25%`
...
- referer请求头  
  ...
  - 之前记得网上说是无法伪造的  
    曾在fetch中测试<span style='opacity:.5'>（详细内容见《JS-前后端共有部分.md》）</span>
...
- 判断当前环境  
  ...
  - MDN说即使是同一浏览器，在升级版本后也有可能更改此值  
  - 浏览器用户是可以更改此值的，也就是UA spoofing
...
增删改查方法很简单，直接通过[文档](https://developer.mozilla.org/zh-CN/docs/Web/API/Storage)了解就行
...
- 跳转
  - 在新页面打开  
    [`window.open('一个url')`](https://developer.mozilla.org/zh-CN/docs/Web/API/Window/open)
    - `window.open('随便什么名字', '_blank')`这种写法也是可以的
    - 会返回新窗口的`window`对象
  - 在本页面打开  
...
> 允许网络应用或者站点，在不借助中间媒介的情况下，建立浏览器之间点对点（Peer-to-Peer）的连接，实现视频流和（或）音频流或者其他任意数据的传输 —— [百度百科](https://baike.baidu.com/item/WebRTC/5522744?fr=aladdin)
...
- 下载  
  ...
  - 可以用a标签下载  
    - 方法A：a标签直接放个url下载
    - 方法B：a标签和[Blob](https://developer.mozilla.org/zh-CN/docs/Web/API/Blob)联动下载   
      - 这个方法在firefox里下载pdf会变成预览  
        这其实是firefox的一个默认设置：检测到下载pdf的行为后改为预览  
        规避方法一：用iframe下载  
        规避方法二：实例化Blob时把`type`设为`'application/vnd.ms-excel'`  
  - 可以用iframe下载
...
- 打印局部内容  
  ...
  - 可以选择开一个新窗口来打印  
    ...
    ```js
    const h = window.open('随便什么名字', '_blank')
    h.document.write('head标签+body内html')
    h.document.close()
    h.print()
    h.close()
    ```
...
- `requestAnimationFrame`  
  `requestAnimationFrame`确实比别的方法好用  
  在cesium里用`requestAnimationFrame`和用`Promise.resolve()`后`then`、`then`、`then`完全不是一个概念
