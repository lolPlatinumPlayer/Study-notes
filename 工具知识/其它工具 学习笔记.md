...
- series数组子项的label属性的formatter属性里的`a`、`b`、`c`
  - `a`  
    series数组子项的`name`属性
  - `b`  
...
可以为自定义 axios实例 添加拦截器

```js
const instance = axios.create();
instance.interceptors.request.use(function () {/*...*/}); // 【】未确定回调触发条件是否和axios对象上的一致
```
...
- ~~拥有代理功能（环境：vue-cli 的dev）~~  
  应该是[http-proxy-middleware](https://github.com/chimurai/http-proxy-middleware#proxycontext-config)吧，[官方](https://cli.vuejs.org/zh/config/#devserver-proxy)说的
...
- 碰撞判断  
  ...
  `booleanContains`和`booleanWithin`的区别：
...
- 判断2个多边形的边缘是否有接触  
  `turf.booleanOverlap(多边形, 多边形)`  
  `booleanOverlap`的官网的解释看不懂  
  但是用多边形测试过后感觉是用来『判断2个多边形的边缘是否有接触』的  
  官网有说可以用于多种几何体
