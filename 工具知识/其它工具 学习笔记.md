...
- 旋转 `transformRotate`  
  ...
  角度区间至少支持(-360,360)  
  - `mutate`选项  
    官网说这个选项设为`true`的话可以提高性能  
    但是个人测试中设为`true`的话会导致变形 —— 版本：`"@turf/transform-rotate": "^5.1.5"`
- 移动
  - `lineOffset`
  - `transformTranslate`
- 缩放 `transformScale`
...
- 将js文件作为node程序启动  
  ...
  - 下拉列表  
    ...
    可以在这里选择不同的js文件
  - “三角形”图标  
    <span style='color:blue'>不</span>进断点地将js文件作为node程序执行
  - “瓢虫”图标  
    <span style='color:blue'>会</span>进断点地将js文件作为node程序执行
