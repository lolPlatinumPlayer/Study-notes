...
- 不同解析器支持情况不同  
  比如Obsidian中`details`标签没有展开状态
- style标签不生效
- html标签中的markdown语法也可以生效  
  不过不同解析器的支持程度不一样  
  比如`details`标签中的单行代码和多行代码在typora中就会出现问题，而在github中就没有问题

### 连写无属性的标签
> 需要注意的是：HTML标签后要加空行，不然部分解析器（比如github）上后续一行的markdown语法会失效
