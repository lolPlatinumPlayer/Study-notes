...
## 正则
- 写法基本都用`/表达式/`这种方法写，虽然`new RegExp(字符串格式表达式)`效果也是一样的，不过后者操作起来有点麻烦
- 判断字符串是否匹配某个表达式
  `表达式.test(字符串)`返回布尔值
- 字符串第一个匹配正则的部分的开头的序号
  `字符串.search(表达式)`
  第一个就匹配到的话就是0，都没匹配到就是-1
- 正则方法后的g：
  不加只会匹配一次，匹配不到返回null，匹配到则返回一个有一个子项的数组，以及一些相关数据。
  加的话会匹配多次，匹配不到返回null，匹配到则返回数组，没有相关数据
  （这里记录的方法中只有replace和match可以用）
- 正则的[]几乎可以把中间所有原来有功能的符号转为无功能的
...
class 类名 {
  ...
  方法名() { // 这个方法会加在（类极其产生的对象的）原型链上（与给`es5构造函数名.prototype.方法名`赋值函数效果一致）
    ...
  }
}
- 存值函数和取值函数
